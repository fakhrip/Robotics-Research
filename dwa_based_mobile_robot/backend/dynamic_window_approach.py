...
class Config:
    ...
    def __init__(self):
        # robot parameter
        self.wheel_radius = 0.125 # [m]
        self.max_speed = 1.0  # [m/s]
        self.min_speed = -0.5  # [m/s]
        self.max_yaw_rate = math.radians(20)  # [rad/s]
        self.max_accel = 0.2  # [m/ss]
        self.max_delta_yaw_rate = math.radians(20)  # [rad/ss]
...
def calc_obstacle_cost(trajectory, ob, config):
    ...
    dx = trajectory[:, 0] - (ox[:, None] if len(ob) > 0 else 0)
    dy = trajectory[:, 1] - (oy[:, None] if len(ob) > 0 else 0)
    r = np.hypot(dx, dy)
    ...
    if config.robot_type == RobotType.rectangle:
        ...
        local_ob = (ob[:, None] if len(ob) > 0 else 0) - trajectory[:, 0:2]
